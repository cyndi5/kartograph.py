...
class SvgRenderer(MapRenderer):
    ...
    def render(self, style, pretty_print=False, render_format='wikiplace'):
        """
        ...
        self.fixed_points=False # whether or not we fixed the points yet
    ...
    def _render_polygon(self, geometry, offset, scale, is_county):
...
#            print 'polygon={0}'.format(polygon)
            ...
            for ring in [polygon.exterior] + list(polygon.interiors):
                ...
                cont_str = ""
                kept = []
                # Initially fixed point to get scaling offsets 
               
                if is_county and not self.fixed_points:
                    temp_pt = ring.coords[0]
                    temp_pt1 = (temp_pt[0]*scale, temp_pt[1]*scale)
...
#            for x in layer.options:
 #               print 'layer.options[{0}]={1}'.format(x,layer.options[x])
            ...
            for feat in layer.features:
                if layer.options['render']:
                    if self.render_format=='wikiplace':
                        #if layer.id=='countylayer':
                        #    is_county=True
                        #else:
                        #    is_county=False
                        node = self._render_feature(feat, layer.options['attributes'],offset=layer.options['offset'], scale=layer.options['scale'],is_county=is_county)
                    else:
                        node = self._render_feature(feat, layer.options['attributes'],offset=layer.options['offset'], scale=layer.options['scale'],is_county=False)
                    if node is not None:
